...
namespace engine {
          ...
          struct Pack {
                    Pack() = default;
                    Pack(uint32_t familyIndex, VkQueue q)
                              :family(familyIndex), queue(q)
                    { }
                    uint32_t family{};
          ...
          struct QueueDispatcher {
                    ...
                    QueueDispatcher(const VkQueueFlagBits queueType, std::vector<Pack>&& normalPool);
                    Pack get_queue();
          ...
          class QueueScheduler {
                    using QueueRequestMappingStruct = std::unordered_map<VkQueueFlagBits, QueueRequest >;
                    using QueueDispatcherMappingStruct = std::unordered_map<
                              VkQueueFlagBits, 
                              std::unique_ptr<QueueDispatcher> 
                    >;
