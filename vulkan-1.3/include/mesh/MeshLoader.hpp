...
namespace engine {
          namespace mesh {
                    ...
                    struct MeshAsset {
                              ...
                              void createAsset(std::vector<Vertex>&& vertices, std::vector<uint32_t>&& indices) {
                                       meshBuffers.createMesh(std::move(vertices), std::move(indices));
                              }

                              void submitMesh(VkCommandBuffer cmd) {
                                        meshBuffers.submitMesh(cmd);
                              }
                              void flushUpload(VkFence fence) {
                                        meshBuffers.flushUpload(fence);
                              }
                              ...
                              loadGltfMeshes(VkDevice device, VmaAllocator allocator, std::filesystem::path filePath);
                    };
          }
}
