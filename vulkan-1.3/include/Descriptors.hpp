...
struct DescriptorPoolManager {
          ...
          static DescriptorPoolManager create(VkDevice device,
          ...
          [[nodiscard]] std::pair<VkDescriptorSet, VkResult>  allocate(VkDescriptorSetLayout layout, void* pNext = VK_NULL_HANDLE);
...
struct DescriptorPoolAllocator {
  ...
  void init(const uint32_t setCount, const std::vector<PoolSizeRatio>& poolSizeRatio);

  void reset_pools();
  void destroy_pools();

  [[nodiscard]] VkDescriptorSet allocate(VkDescriptorSetLayout layout, void* pNext =VK_NULL_HANDLE);

protected:
          [[nodiscard]]  DescriptorPoolManager get_pool();
          void create_pool(const uint32_t setCount, const std::vector<PoolSizeRatio>& poolSizeRatio);
          void init_poolSize(const uint32_t setCount, const std::vector<PoolSizeRatio>& poolSizeRatio);
