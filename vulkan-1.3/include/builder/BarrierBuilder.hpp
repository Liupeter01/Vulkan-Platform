...
namespace details {
template <typename T> struct is_barrier2 : std::false_type {};

template <> struct is_barrier2<VkImageMemoryBarrier2> : std::true_type {};

template <> struct is_barrier2<VkBufferMemoryBarrier2> : std::true_type {};

template <> struct is_barrier2<VkMemoryBarrier2> : std::true_type {};

template <typename T> constexpr bool is_barrier2_v = is_barrier2<T>::value;
} // namespace details
...
struct ImageBarrierBuilder {
  ...
  ImageBarrierBuilder(VkImage image,
                      VkFormat format = VK_FORMAT_B8G8R8A8_UNORM);
  ImageBarrierBuilder &from(VkImageLayout oldLayout);
  ImageBarrierBuilder &to(VkImageLayout newLayout);
  ImageBarrierBuilder &
  aspect(VkImageAspectFlags aspect = VK_IMAGE_ASPECT_COLOR_BIT);
  ImageBarrierBuilder &
  withRange(uint32_t baseMip = 0, uint32_t mipCount = VK_REMAINING_MIP_LEVELS,
            uint32_t baseLayer = 0,
            uint32_t layerCount = VK_REMAINING_ARRAY_LAYERS);

  ImageBarrierBuilder &
  queueIndex(uint32_t srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED,
  ...
  static constexpr std::array<ImageTransitionRule, 32> rules_ = {
      {// UNDEFINED → GENERAL
...

       {
           ...
           VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT,     // or COMPUTE_SHADER_BIT |
                                                     // FRAGMENT_SHADER_BIT
...
private:
  ...
  VkImageSubresourceRange range_{aspectMask_, 0, VK_REMAINING_MIP_LEVELS, 0,
                                 VK_REMAINING_ARRAY_LAYERS};
};

struct BarrierBuilder {
  template <typename T>
  std::enable_if_t<details::is_barrier2_v<std::decay_t<T>>, BarrierBuilder &>
  add(T &&barrier) {
  ...
  std::enable_if_t<(details::is_barrier2_v<std::decay_t<Ts>> && ...),
                   BarrierBuilder &>
  addMany(Ts &&...xs) {
