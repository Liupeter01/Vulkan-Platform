...
namespace engine {
          ...
          struct ParticleLayoutTraits<ParticleT,
                    std::enable_if_t<has_particle_fields_v<ParticleT>>> {
                    static constexpr bool has_position = true;
                    static constexpr bool has_velocity = true;
                    static constexpr bool has_color = true;

                    static constexpr size_t aos_stride = sizeof(ParticleT);

                    static constexpr size_t position_offset =
                              offsetof(ParticleT, position);
                    static constexpr size_t velocity_offset =
                              offsetof(ParticleT, velocity);
                    static constexpr size_t color_offset =
                              offsetof(ParticleT, color);

                    static constexpr size_t position_size =
                              sizeof(decltype(std::declval<ParticleT>().position));
                    static constexpr size_t velocity_size =
          ...
          struct is_valid_particle_layout<
                    ...
                    LayoutT,
                    std::void_t<
                    typename LayoutT<ParticleT>::view
                    >
          ...
          template<typename ParticleT,
                    template<typename> class LayoutT>
          static constexpr bool is_valid_particle_layout_v = is_valid_particle_layout<ParticleT, LayoutT>::value;
}
