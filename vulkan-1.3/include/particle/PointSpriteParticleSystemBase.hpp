...
namespace details {
          ...
          public:
                    PointSpriteParticleSystemBase(VkDevice device)
                              : Compute_EffectBase<ComputePushConstantType, ComputeResourcesType>(device),
                              Graphic_EffectBase< GraphicPushConstantType, GraphicResourcesType>(device),
                              device_(device)
                    {}
          ...
          public:
                    ...
                    virtual void on_gui() {
                              ...
                              if (ImGui::CollapsingHeader("Compute_ParticleSys",
                                        ...
                                        ImGui::SliderFloat("Speed X", &this->getCompPushConstantData().speedup.x, 0.1f,
                                                  5.0f, "%.2fx");
                                        ImGui::SliderFloat("Speed Y", &this->getCompPushConstantData().speedup.y, 0.1f,
                                                  5.0f, "%.2fx");
                                        ImGui::SliderFloat("Speed Z", &this->getCompPushConstantData().speedup.z, 0.1f,
                                                  5.0f, "%.2fx");
                                        ImGui::SliderFloat("Speed W", &this->getCompPushConstantData().speedup.w, 0.1f,
                                                  5.0f, "%.2fx");
                    ...
                    void render(VkCommandBuffer cmd, const MaterialInstance& ins) override {
                              ...
                              vkCmdPushConstants(cmd, ins.pipeline->getPipelineLayout(),
                                        VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(GraphicPushConstantType),
                                        &this->getGraphicPushConstantData());
                    ...
                    void dispatch(VkCommandBuffer cmd, const ComputeInstance& ins) override {
                              ...
                              vkCmdPushConstants(cmd, ins.pipeline->getPipelineLayout(),
                                        VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(ComputePushConstantType),
                                        &this->getCompPushConstantData());
...
class PointSpriteParticleSystemBaseAOS
    : public details::PointSpriteParticleSystemBase<ComputePushConstantType, ComputeResourcesType,
...
public:
  PointSpriteParticleSystemBaseAOS(VkDevice device)
      : BaseClass(device)
  {}
  ...
  ComputeInstance generate_comp_instance(
    ...
    this->computeWriter_.write_buffer(0, resources.particlesIn, resources.bufferSize,
                               0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);

    this->computeWriter_.write_buffer(1, resources.particlesOut, resources.bufferSize,
                               0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
  ...
  MaterialInstance
            generate_graphic_instance(GraphicResourcesType& resources,
                      DescriptorPoolAllocator& globalDescriptorAllocator) override {
            ...
            this->graphicWriter_.write_buffer(0, resources.particlesIn, resources.bufferSize,
                      0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
...
protected:
          ...
          void init_graphic() override {
                    ...
                    /*Graphic Pipeline*/
                    auto layout = { globalSceneLayout_, this->graphicLayout_ };

                    this->defaultOpaquePipeline_.set_vertex_shader(stages[0].path, stages[0].entry);
                    this->defaultOpaquePipeline_.set_fragment_shader(stages[1].path, stages[1].entry);
                    this->defaultOpaquePipeline_.template create<GraphicPushConstantType>(
                              MaterialPass::OPAQUE, layout);

                    this->transparentPipeline_.set_vertex_shader(stages[0].path, stages[0].entry);
                    this->transparentPipeline_.set_fragment_shader(stages[1].path, stages[1].entry);
                    this->transparentPipeline_.template create<GraphicPushConstantType>(
                              MaterialPass::TRANSPARENT, layout);

                    this->defaultOpaquePointPipeline_.set_vertex_shader(stages[0].path, stages[0].entry);
                    this->defaultOpaquePointPipeline_.set_fragment_shader(stages[1].path, stages[1].entry);
...
class PointSpriteParticleSystemBaseSOA 
             : public details::PointSpriteParticleSystemBase<ComputePushConstantType,
                                                                                ComputeResourcesType,
                                                                                GraphicPushConstantType, 
...
public:
          PointSpriteParticleSystemBaseSOA(VkDevice device)
                    : BaseClass(device)
          {}
          ...
          ComputeInstance generate_comp_instance(
                    ...
                    this->computeWriter_.write_buffer(0, resources.positionIn, resources.positionBufferSize,
                                                  0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
                    
                    this->computeWriter_.write_buffer(1, resources.positionOut, resources.positionBufferSize,
                                                  0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
                    
                    this->computeWriter_.write_buffer(2, resources.velocityIn, resources.velocityBufferSize,
                                                  0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
                    
                    this->computeWriter_.write_buffer(3, resources.velocityOut, resources.velocityBufferSize,
                                                  0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
                    
                    this->computeWriter_.write_buffer(4, resources.colorIn, resources.colorBufferSize,
                                                  0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
                    
                    this->computeWriter_.write_buffer(5, resources.colorOut, resources.colorBufferSize,
                                                  0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
          ...
          MaterialInstance
                    ...
                    this->graphicWriter_.write_buffer(0, resources.positionIn, resources.positionBufferSize,
                              0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);

                    this->graphicWriter_.write_buffer(1, resources.colorIn, resources.colorBufferSize,
                              0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
