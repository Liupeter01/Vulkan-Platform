  ...
  std::size_t bufferSize{};
          ...
          public:
                    ...
                    void init() override {
                              ...
                              this->defaultComputePipeline_.set_compute_shader(SLANG_SHADER_PATH"particle2D.slang.spv", "compMain");
                              this->defaultComputePipeline_.template create<PushConstantType>(
                                        ComputePass::DEFAULT, layout);

                              this->specialConstantPipeline_.set_compute_shader(SLANG_SHADER_PATH"particle2D.slang.spv", "compMain");
                              this->specialConstantPipeline_.template create<PushConstantType>(
                                        ComputePass::SPECIALCONSTANT, layout);
                              this->isinit_ = true;
                    }

                    void Compute_ParticleSys2D::on_gui() {

                              if (ImGui::CollapsingHeader("Compute_ParticleSys2D", ImGuiTreeNodeFlags_DefaultOpen)) {
                                        ImGui::Text("Speed Multiplier (0.1x ~ 5x)");

                                        ImGui::SliderFloat("Speed X", &this->pushConstantData_.speedup.x, 0.1f, 5.0f, "%.2fx");
                                        ImGui::SliderFloat("Speed Y", &this->pushConstantData_.speedup.y, 0.1f, 5.0f, "%.2fx");
                                        ImGui::SliderFloat("Speed Z", &this->pushConstantData_.speedup.z, 0.1f, 5.0f, "%.2fx");
                                        ImGui::SliderFloat("Speed W", &this->pushConstantData_.speedup.w, 0.1f, 5.0f, "%.2fx");
