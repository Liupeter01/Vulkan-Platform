...
namespace details {
struct BufferArray2 {
  BufferArray2(VkDevice dev, VmaAllocator alloc,
               std::string name = "BufferArray2");
  virtual ~BufferArray2();

  void create();
  void recreate(const std::size_t allocSize, VkBufferUsageFlags bufferUsage,
                VmaMemoryUsage memoryUsage);

  void clear();

  template <typename T> void perpareTransferData(std::vector<T> &&data) {
...
template <typename ParticleT, typename = void> struct AOSLayout;

template <typename ParticleT>
struct AOSLayout<ParticleT,
                 std::enable_if_t<has_particle_fields_v<ParticleT>, void>> {
  ...
  AOSLayout(VkDevice device, VmaAllocator allocator,
            std::string name = "AOSLayout")
      : device_(device), allocator_(allocator), name_(name),
  ...
  void configure(const std::size_t particleCount,
    ...
    packed_.configure(Traits::aos_stride * particleCount, bufferUsage,
                      memoryUsage);
...
template <typename ParticleT, typename = void> struct SOALayout;

template <typename ParticleT>
struct SOALayout<ParticleT,
                 std::enable_if_t<has_particle_fields_v<ParticleT>, void>> {
  ...
  struct LayoutView {
    LayoutView(VkBuffer pos, VkBuffer v, VkBuffer c, std::size_t pBufferSize,
               std::size_t vBufferSize, std::size_t cBufferSize)
        : position(pos), velocity(v), color(c), positionBufferSize(pBufferSize),
          velocityBufferSize(vBufferSize), colorBufferSize(cBufferSize) {}
  ...
  SOALayout(VkDevice device, VmaAllocator allocator,
            std::string name = "SOALayout")
      : device_(device), allocator_(allocator), name_(name),
        position_(device, allocator), velocity_(device, allocator),
        color_(device, allocator) {}

  virtual ~SOALayout() { destroy(); }
  ...
  void configure(const std::size_t particleCount,
    ...
    position_.configure(Traits::position_size * particleCount, bufferUsage,
                        memoryUsage);

    velocity_.configure(Traits::velocity_size * particleCount, bufferUsage,
                        memoryUsage);

    color_.configure(Traits::color_size * particleCount, bufferUsage,
                     memoryUsage);
  ...
  static void splitSOA(const ParticleType *__restrict src, size_t count,
                       glm::vec4 *__restrict pos, glm::vec4 *__restrict vel,
  ...
  LayoutView out(bool idx) const noexcept {
    return {position_.out(idx),        velocity_.out(idx),
            color_.out(idx),           position_.getBufferSize(),
            velocity_.getBufferSize(), color_.getBufferSize()};
  }
