...
namespace v2 {
template <typename ParticleType, template <typename, typename> class LayoutT,
          typename = void>
...
template <typename ParticleType, template <typename, typename> class LayoutT>
class ParticleSysDataBuffer2<
    ParticleType, LayoutT,
    std::enable_if_t<is_valid_particle_layout_v<ParticleType, LayoutT>, void>> {

public:
  using Layout = LayoutT<ParticleType, void>;
  using View = typename Layout::view;

  ParticleSysDataBuffer2(VkDevice device, VmaAllocator allocator,
                         std::string name = "ParticleSysDataBuffer2")
...
public:
  ...
  std::size_t getParticleCount() const noexcept { return particleCount_; }
  ...
  void prepareTransferData() {
    ...
    auto data = ((dim_ == ParticleDimension::Particle2D) ? fill2D() : fill3D());
...
private:
  void __createGpuBuffer() {
    ...
    layout_.reset();
    layout_ = std::make_unique<Layout>(device_, allocator_,
                                       name_ + std::string("::Layout"));
