...
namespace engine {
          ...
          namespace mesh {
                    ...
                    GPUGeoMeshBuffers& GPUGeoMeshBuffers::createMesh(
                              const Mesh& mesh){

                              const std::size_t vertexBufferSize = mesh.vertices.size() * sizeof(mesh.vertices[0]);
                              const std::size_t indiciesBufferSize = mesh.indices.size() * sizeof(mesh.indices[0]);
                    ...
                    void GPUGeoMeshBuffers::submitMesh(VkCommandBuffer cmd) {
                              ...
                              const std::size_t indiciesBufferSize = indicies_.size() * sizeof(indicies_[0]);
                              ...
                              memcpy(reinterpret_cast<unsigned char*>(src), vertex_.data(), vertexBufferSize);
                              memcpy(reinterpret_cast<unsigned char*>(src) + vertexBufferSize, indicies_.data(), indiciesBufferSize);
                              staging.unmap();
                    ...
                    void GPUGeoMeshBuffers::flushUpload(VkFence fence) {
                              ...
                              vkWaitForFences(device_, 1, &fence, true, std::numeric_limits<uint64_t>::max());
