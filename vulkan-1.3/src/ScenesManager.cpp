...
void ScenesManager::init_default_material(){
          ...
          if (!metalRoughMaterial) {
                    spdlog::error("[ScenesManager Error]: Create Default Graphic Material Failed!");
...
void ScenesManager::init_default_compute() {
          ...
          if (!imageAttachmentCompute) {
                    spdlog::error("[ScenesManager Error]: Create Default Compute Material Failed!");
...
bool ScenesManager::addScene(std::shared_ptr<NodeManager> scene) {
          if (scene->name.empty()) {
                    spdlog::warn("[ScenesManager Warn]: No Scene Name Found! Create As default!");
          }
          auto [_, status] = loadedScenes_.try_emplace(scene->name, scene);
          if (!status) {
                    spdlog::warn("[ScenesManager Warn]: Insert Scene Failed Due to Node Already Existed!");
          }
          return status;
}
