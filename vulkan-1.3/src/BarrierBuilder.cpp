...
namespace engine {
          ImageBarrierBuilder::ImageBarrierBuilder(VkImage image, VkFormat format)
                    :image_(image),
                    srcQueueFamilyIndex_(VK_QUEUE_FAMILY_IGNORED),
          ...
          ImageBarrierBuilder& ImageBarrierBuilder::queueIndex(uint32_t srcQueueFamilyIndex,
          ...
          VkImageMemoryBarrier2 ImageBarrierBuilder::build() {
                    ...
                    throw std::runtime_error(fmt::format(
                              "Unsupported layout transition: {} -> {}", int(oldLayout_), int(newLayout_)));
          }

          void BarrierBuilder::addBarrier(const VkMemoryBarrier2& b) { memBarriers_.push_back(b); }
          void BarrierBuilder::addBarrier(const VkBufferMemoryBarrier2& b) { bufBarriers_.push_back(b); }
          void BarrierBuilder::addBarrier(const VkImageMemoryBarrier2& b) { imgBarriers_.push_back(b); }
