...
namespace engine {
          namespace node {
                    ...
                    void NodeManager::create_root(const std::string& root_name) {
                              if (scene_root_) {
                                        spdlog::warn("[Scene Warn]: Replacing existing root node '{}'", scene_root_->node_name);
                                        remove_nodes(scene_root_); 
                              }
                    ...
                    bool NodeManager::attachChildren(const std::string& parentName, std::shared_ptr<MeshAsset> asset) {
                              spdlog::debug("[Scene Info] Attaching single MeshAsset child '{}' to parent '{}'", asset->meshName, parentName);
                              return attachChildrens(parentName, { asset });
                    }

                    bool NodeManager::attachChildren(const std::string& parentName, std::shared_ptr<node::BaseNode> child) {
                              spdlog::debug("[Scene Info] Attaching single BaseNode child '{}' to parent '{}'", child->node_name, parentName);
                              return attachChildrens(parentName, { child });
                    }

                    bool NodeManager::attachChildrens(const std::string& parentName,
                              ...
                              for (auto& node : childrens) {
                                        if (!node) {
                                                  spdlog::warn("[Scene Warn]: Nullptr child detected under parent {}", parentName);
                                                  continue;
                                        }

                                        //it's inside insert function: parentNode->children.push_back(child);
                                        if (!insert(parentNode.value(), node)) {
                                                  spdlog::warn("[Scene Warn]: Failed to attach child node: {}", node->node_path);
                                                  status = false;
                                        }
                              }
                              return status;
                    ...
                    bool NodeManager::attachChildrens(const std::string& parentName,
                              ...
                              for (auto& node : childrens) {
                                        ...
                                        if (!insert(parentNode.value(), child)) {
                                                  spdlog::warn("[Scene Warn]: Failed to attach child node: {}", child->node_path);
                                                  status = false;
                                        }
                              }
                              return status;
                    ...
                    std::optional < std::shared_ptr<node::BaseNode>> NodeManager::findNode(const std::string& name) const {
                    ...
                    std::optional < std::shared_ptr<MeshAsset>> NodeManager::findMesh(const std::string& name) const {
                    ...
                    bool NodeManager::insert_to_lookup(std::shared_ptr<node::BaseNode> node, const std::string& parentPath) {
                    ...
                    bool NodeManager::insert(std::shared_ptr<node::BaseNode> parent, std::shared_ptr<node::BaseNode> child) {
                              if (!parent || !child) {
                              ...
                              if (!insert_to_lookup(child, parent->node_path)) {
                                        spdlog::warn("[Scene Warn]: Node {} already exists in lookup.", child->node_path);
                                        return false;
                              }
                              return true;
                    }
