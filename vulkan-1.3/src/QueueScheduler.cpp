...
namespace engine {
          ...
          QueueDispatcher::QueueDispatcher(const VkQueueFlagBits queueType, std::vector<Pack>&& normalPool)
                    :normalPools_(std::move(normalPool))
                    , queueType_(queueType)
                    , normalIndex(0)
                    , normalCounter(normalPool.size())
          { 
                    if (normalCounter < 1) 
          ...
          Pack QueueDispatcher::get_queue() {
                    return normalPools_[dispatch()];
          }

          VkQueueFlagBits QueueDispatcher::getQueueType() const {
                    return queueType_;
          }

          QueueScheduler::QueueScheduler(VkDevice device, const QueueRequestMappingStruct& request)
          ...
          void QueueScheduler::finalizeByCategory(VkQueueFlagBits flag) {
                    ...
                    if (dispatcher->normalPools_.empty()) {
                              throw std::runtime_error("[QueueScheduler] No queues available for graphics family!");
                    }
                    ...
                    if (available < minimum_required) {
                              auto msg = fmt::format(
                                        "[QueueScheduler] Insufficient graphics queues: "
                    ...
                    if (available <= 1) {
                              spdlog::warn(
                                        "[QueueScheduler] Only {} graphics queue left. "
          ...
          void QueueScheduler::extractQueuesByCategory(VkQueueFlagBits flag){
                    ...
                    if (request.allocatedQueueInfo.empty()) {
                              spdlog::warn("[QueueScheduler] No allocation found for flag {}", uint32_t(flag));
                              return;
                    }

                    for (const auto& [family, range] : request.allocatedQueueInfo) {
                              for (uint32_t startIndex = range.startPos; startIndex < range.startPos + range.count; ++startIndex) {
                                        VkQueue queue;
                    ...
                    auto [_, status] = queueDispatcher_.try_emplace(flag, 
                              std::make_unique< QueueDispatcher>(flag, std::move(normalPools)));
                    if (!status) {
                              spdlog::error("[QueueScheduler Error]: Init {} Queue's Dispatcher Failed!", static_cast<uint32_t>(flag));
                    }
          }
