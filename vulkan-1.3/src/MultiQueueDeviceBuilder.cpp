...
MultiQueueDeviceBuilder::MultiQueueDeviceBuilder(vkb::DeviceBuilder &builder,
    ...
    : builder_{builder}, pdev_(pdev), pq_graphics(cmp), pq_compute(cmp),
      pq_transfer(cmp) {
...
MultiQueueDeviceBuilder &
MultiQueueDeviceBuilder::graphics(uint32_t count, bool presentQueueSupport,
                                  bool reserveImgui) {
...
vkb::Device MultiQueueDeviceBuilder::build() {
  ...
  std::vector<vkb::CustomQueueDescription> descs =
      build_descriptions_from_blockPool();
...
std::list<MultiQueueDeviceBuilder::QueueBlock>
MultiQueueDeviceBuilder::collectQueueFamilies() {
...
typename MultiQueueDeviceBuilder::PQ
MultiQueueDeviceBuilder::BuildPQ(VkQueueFlagBits requiredFlags) {
...
typename MultiQueueDeviceBuilder::PQ &
MultiQueueDeviceBuilder::select_pq(VkQueueFlagBits requiredFlags) {
...
void MultiQueueDeviceBuilder::consume(VkQueueFlagBits requiredFlags,
                                      QueueRequest &req) {
  ...
  while (requested > 0) {
    ...
    QueueBlock &block = *node.block; // list iterator → actual block
    ...
    req.allocatedQueueInfo.emplace_back(
        block.familyIndex, QueueRequest::Span{block.startPos, alloc});
