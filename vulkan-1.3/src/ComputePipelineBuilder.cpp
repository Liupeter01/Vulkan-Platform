...
VkPipeline ComputePipelineBuilder::build() {
  ...
  if (hasSpecialization_) {
    specializationInfo_.mapEntryCount =
        static_cast<uint32_t>(specializationEntries_.size());
...
ComputePipelineBuilder &ComputePipelineBuilder::set_shaders_stage(
    const ComputePipelineBuilder::ShaderStageDesc &stage) {

  computeShader = tools::shader_stage_create_info(device_, stage.path,
                                                  stage.stage, stage.entry);

  return *this;
}

ComputePipelineBuilder &ComputePipelineBuilder::set_specialization_constant(
    uint32_t constantID, const void *data, size_t size) {
  if (!data || size == 0)
    throw std::runtime_error(
        "ComputePipelineBuilder: invalid specialization constant data.");
