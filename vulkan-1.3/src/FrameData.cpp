...
namespace engine {
          ...
          FrameData::FrameData(VkDevice device)
                    :_device(device), _frameDescriptor(device), isCommandInit(false), isSyncInit(false)
          {}
          ...
          void FrameData::init_command(const VkCommandPoolCreateInfo& commandPoolInfo) {
                    if (isCommandInit) return;

                    vkCreateCommandPool(_device, &commandPoolInfo, nullptr,
                              &_commandPool);
          ...
          void  FrameData::reset_allocator_pools() {
          ...
          void FrameData::clean_last_frame() {
                    _deletionQueue.flush();
          }

          void FrameData::init_allocator(const uint32_t setCount, const std::vector<PoolSizeRatio>& poolSizeRatio) {
                    _frameDescriptor.init(setCount, poolSizeRatio);
          }

          void FrameData::destroy_allocator() {
