...
void FrameData::init_command(const VkCommandPoolCreateInfo &commandPoolInfo) {
  ...
  vkCreateCommandPool(engine_->device_, &commandPoolInfo, nullptr, &_commandPool);

  VkCommandBufferAllocateInfo cmdAllocInfo =
      tools::command_buffer_allocate_info(_commandPool, 1);

  vkAllocateCommandBuffers(engine_->device_, &cmdAllocInfo, &_mainCommandBuffer);
...
void FrameData::init_sync(const VkFenceCreateInfo &fenceCreateInfo,
  ...
  vkCreateFence(engine_->device_, &fenceCreateInfo, nullptr, &_renderFinishedFence);
  vkCreateSemaphore(engine_->device_, &semaphoreCreateInfo, nullptr, &_swapChainWait);
...
void FrameData::init_images(VkExtent3D extent) {
          ...
          drawImage_ = std::make_unique<AllocatedImage>(engine_->device_, engine_->allocator_);
          depthImage_ = std::make_unique<AllocatedImage>(engine_->device_, engine_->allocator_);
...
void FrameData::reset_images(VkExtent3D newExtent) {
          ...
          spdlog::info("[FrameData]: Windows Resize width = {}, height = {}, depth = {}",
