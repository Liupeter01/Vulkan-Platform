...
namespace engine {
          GraphicPipelineBuilder::GraphicPipelineBuilder(VkDevice device) :device_(device) {
                    clear();
          ...
          void GraphicPipelineBuilder::clear() {
                    ...
                    colorBlendAttachment_.colorWriteMask = VK_COLOR_COMPONENT_R_BIT |
          ...
          GraphicPipelineBuilder& GraphicPipelineBuilder::set_color_attachment_format(VkFormat format) {
          ...
          GraphicPipelineBuilder& GraphicPipelineBuilder::set_depth_format(VkFormat format) {
                    renderInfo_.depthAttachmentFormat = format;
                    return *this;
          }

          GraphicPipelineBuilder& GraphicPipelineBuilder::set_shaders(const std::string& vertexShaderPath,
                    const std::string& fragmentShaderPath) {
                    shaderStages_.clear();
                    shaderStages_.resize(2);

                    shaderStages_[0] =
                              std::move(tools::shader_stage_create_info(device_, vertexShaderPath, VK_SHADER_STAGE_VERTEX_BIT));
                    shaderStages_[1] =
                              std::move(tools::shader_stage_create_info(device_, fragmentShaderPath, VK_SHADER_STAGE_FRAGMENT_BIT));
                    return *this;
          }

          GraphicPipelineBuilder& GraphicPipelineBuilder::set_input_topology(const VkPrimitiveTopology topology, const bool restart) {
          ...
          GraphicPipelineBuilder& GraphicPipelineBuilder::set_polygon_mode(const VkPolygonMode mode, const float linewidth) {
                    rasterizer_.polygonMode = mode;
                    rasterizer_.lineWidth = 1.f;
                    return *this;
          }

          GraphicPipelineBuilder& GraphicPipelineBuilder::set_cull_mode(const VkCullModeFlags cullMode, const VkFrontFace frontFace) {
