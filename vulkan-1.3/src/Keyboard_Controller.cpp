...
namespace engine {
          void KeyBoardController::movePlaneYXZ(GLFWwindow* win, float dt,
                    ...
                    if (!obj) {
                              spdlog::error("[KeyBoardController movePlaneYXZ]: Invalid BaseNode Deteced!");
                    ...
                    if (glm::length(rotate_camera) > std::numeric_limits<float>::epsilon()) {
                              obj->localTransform.rotation += lookSpeed * dt * glm::normalize(rotate_camera);
                    }

                    // RAD
                    obj->localTransform.rotation.x = glm::clamp(
                              obj->localTransform.rotation.x, -glm::half_pi<float>(), glm::half_pi<float>());
                    obj->localTransform.rotation.y =
                              glm::mod(obj->localTransform.rotation.y, glm::two_pi<float>());

                    auto rotation_matrix =
                              glm::eulerAngleYXZ(obj->localTransform.rotation.y, obj->localTransform.rotation.x,
