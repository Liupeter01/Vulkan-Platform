...
namespace engine {
          ...
          AsyncSubmitHandler::AsyncSubmitHandler(VkQueueFlagBits type, std::size_t queueCount)
                    :currentIndex{ 0 }
                    , queueCounter_(queueCount)
                    , type_(type)
          {}

          AsyncSubmitHandler::AsyncSubmitHandler(VkQueueFlagBits whichTypeIWant, const QueueScheduler& s)
                    :currentIndex{ 0 } 
          {
          ...
          void AsyncSubmitHandler::start() {
                    ...
                    if (queueCounter_ <= 0) {
                              spdlog::warn("[AsyncSubmitHandler Warn]: Illegal Queue Count Detected! Automatically set to one");
         ...
         std::size_t AsyncSubmitHandler::dispatch() {
                   ...
                   auto str = fmt::format("[AsyncSubmitHandler Error]: QueueInfo Submission Error!"
