...
DescriptorPoolManager DescriptorPoolManager::create(VkDevice device,
...
void DescriptorPoolManager::init(const uint32_t set_count,
          const std::vector<VkDescriptorPoolSize>& poolSizes) {
...
{ }
 ...
 void DescriptorPoolAllocator::init(const uint32_t setCount, const std::vector<PoolSizeRatio>& poolSizeRatios) {
...
void DescriptorPoolAllocator::create_pool(
          ...
          readyPools_.push_back(DescriptorPoolManager::create(device_, setCount, poolSizes_));
...
void DescriptorPoolAllocator::init_poolSize(const uint32_t setCount, const std::vector<PoolSizeRatio>& poolSizeRatio) {
          setCount_ = setCount;
...
VkDescriptorSet DescriptorPoolAllocator::allocate(VkDescriptorSetLayout layout, void* pNext) {
...
void DescriptorWriter::write_buffer(int binding, VkBuffer buffer, size_t size, size_t offset, VkDescriptorType type) {
          ...
          auto& ref = bufferInfos.emplace_back(VkDescriptorBufferInfo{ buffer, offset, size });
...
void DescriptorWriter::write_image(int binding, VkImageView imageView, VkSampler sampler, VkImageLayout layout, VkDescriptorType type) {
          ...
          auto& ref = imageInfos.emplace_back(VkDescriptorImageInfo{ sampler, imageView, layout });
