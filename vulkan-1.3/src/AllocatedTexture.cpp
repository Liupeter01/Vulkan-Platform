...
VkImage AllocatedTexture2::image() { return image_; }

VkImageView AllocatedTexture2::imageView() { return imageView_; }
...
void AllocatedTexture2::__createGpuImage() {
  ...
  if (mipMapped_) {
    rimg_info.mipLevels = static_cast<uint32_t>(util::generate_mipmap_levels(
...
void AllocatedTexture2::perpareTransferData(const void *data,
...
void AllocatedTexture2::updateCpuBuffer(const void *data,
                                        const std::size_t length) {
  if (!configured_)
...
void AllocatedTexture2::updateCpuStaging() {
  ...
  std::memcpy(reinterpret_cast<unsigned char *>(src), textureBuffer_.data(),
              textureBuffer_.size());
  staging_.unmap();
...
void AllocatedTexture2::recordUpload(VkCommandBuffer cmd) {
  ...
  if (mipMapped_) {
    util::generate_mipmaps(cmd, image(),
                           {imageExtent_.width, imageExtent_.height});
  } else {
    util::transition_image(cmd, image(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                           VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  }
...
void AllocatedTexture2::configure(VkExtent3D extent, VkFormat format,
  ...
  textureSize_ = imageExtent_.depth * imageExtent_.height * imageExtent_.width *
                 tools::bytes_per_pixel(imageFormat_);
